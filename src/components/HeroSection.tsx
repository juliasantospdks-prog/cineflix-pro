...
const HeroSection = ({ onOpenChat, onPlayTrailer, movies }: HeroSectionProps) => {
  ...
  return (
    <section ref={containerRef} className="relative h-[90vh] min-h-[650px] flex items-end overflow-hidden">
      {/* Animated background images with parallax + Ken Burns */}
      <motion.div className="absolute inset-0 z-0" style={{ y: smoothY, scale: smoothScale }}>
      ...
      {/* Ambient glow */}
      <motion.div 
        className="absolute bottom-0 left-0 w-[40%] h-[40%] bg-cinema-red/10 blur-[120px] z-[1]"
        animate={{ 
          opacity: [0.3, 0.5, 0.3],
          scale: [1, 1.1, 1],
        }}
      ...
      {heroMovies.length > 1 && (
        <>
          <motion.button 
            ...
            className="absolute left-4 top-1/2 -translate-y-1/2 z-10 w-12 h-12 rounded-full bg-black/40 hover:bg-cinema-red/80 backdrop-blur-sm flex items-center justify-center transition-all duration-300 group"
          ...
          <motion.button 
            ...
            className="absolute right-4 top-1/2 -translate-y-1/2 z-10 w-12 h-12 rounded-full bg-black/40 hover:bg-cinema-red/80 backdrop-blur-sm flex items-center justify-center transition-all duration-300 group"
            whileHover={{ scale: 1.1, x: 3 }}
            whileTap={{ scale: 0.95 }}
          >
      ...
      <div className="relative z-[2] container mx-auto px-4 pb-24">
        <AnimatePresence mode="wait">
          ...
          >
            {/* Badge */}
            <motion.div className="flex items-center gap-3 mb-4" variants={textItemVariants}>
            ...
            {/* Title */}
            <motion.h1 className="font-cinema text-4xl md:text-6xl lg:text-7xl text-white mb-3 leading-none glow-text" variants={textItemVariants}>
              {title}
            </motion.h1>

            {/* Meta info */}
            <motion.div className="flex items-center gap-4 mb-4 flex-wrap" variants={textItemVariants}>
            ...
            {/* Synopsis */}
            <motion.p className="text-base md:text-lg text-white/70 mb-6 leading-relaxed line-clamp-3 max-w-xl" variants={textItemVariants}>
              {current.overview}
            </motion.p>

            {/* CTA Buttons */}
            <motion.div className="flex flex-wrap gap-3" variants={textItemVariants}>
              <motion.div
